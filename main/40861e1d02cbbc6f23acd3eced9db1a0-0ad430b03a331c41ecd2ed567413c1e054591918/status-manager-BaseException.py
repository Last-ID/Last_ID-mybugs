...
while 1:
    ...
    try:
        datatxt = open('data.txt','a',encoding='utf-8')
        status_ping = StatusPing('play.zamcraft.net')
        ping_result = str(status_ping.get_status())
        #print(status_ping.get_status())
        datatxt.write('\n')
        datatxt.write(time.strftime("%Y-%m-%d--%H:%M:%S", time.localtime()) +"--"+ str(int(time.time())) + ping_result)
        ...
        print("[Info]Error!")
        print("[Info][Debug]ping exception: %s: %s" %(e.errno, e.strerror))
    else:
        print("Success!")
