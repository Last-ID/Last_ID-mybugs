...
class StatusPing:
    ...
    def __init__(self, host='locatehost', port=25565, timeout=5):
...
while 1:
    try:
        ...
        status_ping = StatusPing('play.miaoscraft.cn')
        ping_data=status_ping.get_status()

        json_str = json.dumps(ping_data)
        #loads:把json转换为dict
        ping_info = json.loads(json_str)
        get_max = jsonpath.jsonpath(ping_info, '$.players.max')
        get_online = jsonpath.jsonpath(ping_info, '$.players.online')
        print(time.strftime("%a %b %d %H:%M:%S %Y", time.localtime()) + str(get_online) + "/" + str(get_max)+str(str(get_online)<str(get_max)))
        time.sleep(1)
    except IOError as e:
